package com.example.solarsystemapp.controller
...
import com.example.solarsystemapp.model.ModelSurfaceView
import com.example.solarsystemapp.scene.SceneLoader

class TouchController(view: ModelSurfaceView, renderer: ModelRenderer) {
    private val view: ModelSurfaceView = view
    private val mRenderer: ModelRenderer
    ...
    @Synchronized
    fun onTouchEvent(motionEvent: MotionEvent): Boolean { // MotionEvent reports input details from the touch screen
...
// interested in events where the touch position changed.
        ...
        if (pointerCount == 1 && simpleTouch) {
            val scene: SceneLoader? = view.modelActivity.scene
            scene?.processTouch(x1, y1)
        }
        val max: Int = Math.max(mRenderer.width, mRenderer.height)
        if (touchDelay > 1) { // INFO: Process gesture
            val scene: SceneLoader? = view.modelActivity.scene
            ...
            if (pointerCount == 1 && currentPress1 > 4.0f) {
                ...
                if (fingersAreClosing) {
                    ...
                    val zoomFactor: Float =
                        (length - previousLength) / max * mRenderer.getFar()
                    Log.v(
                        TAG,
                        "Zooming '$zoomFactor'..."
                    )
...
internal class TouchScreen {
    ...
    fun onTouch(
        v: View,
        event: MotionEvent
    ): Boolean { // handle touch events here
